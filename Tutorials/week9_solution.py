...
class SampleApp(object):
    def __init__(self, master):
        ...
        btnframe = tk.Frame(master)
        btnframe.pack(side=tk.TOP)
        # When this button is clicked, the label colour should change to blue.
        btn = tk.Button(btnframe, text="Change to Blue", command=self.change_blue)
        btn.pack(side=tk.LEFT)
        # When this button is clicked, the label colour should change to green.
        btn = tk.Button(btnframe, text="Change to Green",
                     command=self.change_green)
        btn.pack(side=tk.LEFT)

        entryfrm = tk.Frame(master)
        entryfrm.pack(side=tk.TOP, fill=tk.X, padx=10, pady=10)
        entLbl = tk.Label(entryfrm, text="Change the colour to: ")
        entLbl.pack(side=tk.LEFT)
        self._colour_entry = tk.Entry(entryfrm)
        ...
        chngBtn = tk.Button(entryfrm, text="Change it!",
               command=self.change)
        chngBtn.pack(side=tk.LEFT)
