...
if __name__ == '__main__':
    # -----------------------------------------------------------------------------
    # Notice the difference between each version of uniqify, and that it
    # increases significantly with each power of 10.
    # 
    # Note also that uniqify will take a considerable amount of time to
    # complete for 64000 items.
    # 
    ...
    for func, number, order in FUNCTIONS:
        ...
        for N in [1000, 2000, 4000, 8000, 16000, 32000, 64000]:
            time = min(timeit.repeat(func+'(range(N))', 
                'from __main__ import N,'+func, number=number))*1000/number
            print ('n = {:>5}: {:.4f} ms'.format(N, time) +
                   ('' if old is None else '   = {:.2f} times slower'.format(\
                           time/old)))
            old = time

        print("Extrapolating:")
        bigN = 10**6
        print("n = {:,>5}: {:.2f} seconds".format(bigN, time * order(bigN) / \
                order(N) / 1000))
